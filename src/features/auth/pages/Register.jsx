...
export default function Register() {
  ...
  const handleFincaStep = async ({ fincaNombre, nombreAdmin, acceptsTerms, legalVersion }) => {
    setSubmitting(true);
    setError('');
    try {
      ...
      {step === 1 && (
        <>
          ...
          <form onSubmit={handleAccountStep} className="auth-form" noValidate>
            ...
            {/* Passive notice: the EXPLICIT acceptance (checkbox) lives in the
                create-organization step, where the contract is born. */}
            <p className="auth-legal-note">
              Al continuar aceptás los <Link to="/terms" target="_blank" rel="noopener">Términos</Link> y la{' '}
              <Link to="/privacy" target="_blank" rel="noopener">Política de privacidad</Link>.
            </p>
          </form>
        </>
      )}
