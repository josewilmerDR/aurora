...
const CHAT_TOOLS = [
  ...
  {
    name: 'crear_empleado',
    description: 'Crea un registro de persona en el sistema. Una persona puede ser usuario del sistema (con acceso a la app), empleado en planilla, o ambos. Al menos una de las dos facetas debe ser true. SIEMPRE pide confirmación antes de llamar esta herramienta.',
    input_schema: {
      type: 'object',
      properties: {
        nombre:           { type: 'string', description: 'Nombre completo.' },
        email:            { type: 'string', description: 'Correo electrónico. Obligatorio si tieneAcceso=true, opcional si no.' },
        telefono:         { type: 'string', description: 'Número de teléfono (opcional).' },
        rol:              { type: 'string', enum: ['trabajador', 'encargado', 'supervisor', 'rrhh', 'administrador'], description: 'Rol del usuario en el sistema. Sólo aplica si tieneAcceso=true.' },
        tieneAcceso:      { type: 'boolean', description: 'true si la persona debe poder iniciar sesión en la app. Requiere email y rol válidos. Default: false.' },
        empleadoPlanilla: { type: 'boolean', description: 'true si la persona está en planilla y debe tener ficha laboral. Default: false.' },
      },
      required: ['nombre'],
    },
  },
  {
    ...
    description: 'Modifica los datos de una persona existente. Para "rescindir contrato" pasa empleadoPlanilla=false. Para "quitar acceso al sistema" pasa tieneAcceso=false. No puede dejar a la persona sin ninguna de las dos facetas. SIEMPRE pide confirmación.',
    input_schema: {
      type: 'object',
      properties: {
        empleadoId:       { type: 'string', description: 'ID Firestore de la persona a modificar.' },
        ...
        rol:              { type: 'string', enum: ['trabajador', 'encargado', 'supervisor', 'rrhh', 'administrador'], description: 'Nuevo rol en el sistema (opcional). Sólo aplica si tieneAcceso=true.' },
        tieneAcceso:      { type: 'boolean', description: 'Nuevo estado de acceso al sistema. false desactiva login y borra membership. true requiere email y rol válidos.' },
        empleadoPlanilla: { type: 'boolean', description: 'Nuevo estado de planilla. true = asignado/recontratado; false = rescindir contrato (los registros laborales se conservan).' },
      },
      required: ['empleadoId'],
    },
  },
