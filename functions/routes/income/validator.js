// Validación pura del payload de `income_records`. Pensado para ser testeable:
// no toca Firestore, no verifica existencia de IDs (eso lo hace el handler).
...
// Formato ISO estricto — rechaza "2026-02-30" que new Date() aceptaría.
...
// Dado un cuerpo validado (o validable), devuelve { error } o { data } listo
// para persistir. `buyerName` se espera ya resuelto por el handler.
function buildIncomeDoc(body, { buyerName = '' } = {}) {
  ...
  // Si el cliente envía totalAmount, lo respetamos (puede haber descuentos);
  // si no, lo calculamos. Límite defensivo para evitar overflow.
  ...
  // Coherencia: no puede estar "cobrado" sin fecha real de cobro.
