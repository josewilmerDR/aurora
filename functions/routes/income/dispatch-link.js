// Convierte un `cosecha_despachos` en un `income_records` pre-poblado.
// No crea el registro por sí solo — devuelve un borrador que el frontend puede
// completar con buyer + precio antes de hacer POST /api/income.
...
async function draftFromDispatch(req, res) {
  try {
    ...
    // Borrador — el frontend completa buyerId, unitPrice y expectedCollectionDate.
    res.json({
      draft: {
        date: d.fecha,
