...
router.get('/api/hr/planilla-fijo', authenticate, rateLimit('hr_planilla_read', 'costly_read'), async (req, res) => {
  try {
    ...
    // Minimización de PII (auditoría FixedPayrollHistory): el historial ve un
    // empleado a la vez. view=index → metadata + trabajadorIds (membresía, sin
    // dinero); empleadoId=X → metadata + solo la fila de ESE empleado. Sin
    // parámetros devuelve filas completas (lo usa el editor).
